...
void task_add_current_tick(uint64_t tick) {
    ...
    if (!current || !sched_class || !sched_class->update_tick) return;
    sched_class->update_tick(current, tick);
}
...
task_struct *task_copy(struct task_struct *task, task_flags flags) {
    ...
    // 初始化调度器私有数据
    if (sched_class && sched_class->sched_init) sched_class->sched_init(new_task);
    ...
    // 加入调度队列
    if (sched_class && sched_class->enqueue) sched_class->enqueue(new_task);
...
void task_sched(void) {
    ...
    if (sched_class && sched_class->pick_next) next = sched_class->pick_next();
    if (!next) goto out;
